...
export class Mapping<I, O> {
    ...
    public async withValidation( mappingFunction : ( input : I ) => Promise<O> ) : Promise<O> {
    ...
    public withNoValidation( mappingFunction : ( input : I ) => O ) : O {
        this.checkClasses();
        const inputInstance  = plainToClass( this.inputClass!, this.inputPlain );
        const outputPlain    = mappingFunction( inputInstance );
        return plainToClass( this.outputClass!, outputPlain );
    }
